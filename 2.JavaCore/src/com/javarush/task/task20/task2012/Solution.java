...
public class Solution {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        OutputToConsole ots = new OutputToConsole(1);

        FileOutputStream fileOutput = new FileOutputStream("c:/tmp/2012.txt");
        ObjectOutputStream objectOutput = new ObjectOutputStream(fileOutput);
        fileOutput.close();
        objectOutput.close();

        FileInputStream fileInput = new FileInputStream("c:/tmp/2012.txt");
        ObjectInputStream objectInput = new ObjectInputStream(fileInput);
        OutputToConsole ots1 = (OutputToConsole) objectInput.readObject();
        fileInput.close();
        objectInput.close();
    }
}
